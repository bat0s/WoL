...
#define error(...)                                                                                                                                  \
    do {                                                                                                                                            \
        (fprintf(stderr, "\x1B[31m[ERROR] (%s: %d: %s) ", __FILE__, __LINE__, __func__), fprintf(stderr, __VA_ARGS__), fputs("\x1b[0m\n", stderr)); \
        exit(EXIT_FAILURE);                                                                                                                         \
    } while (0)
...
static unsigned char* mac_from_ipv4(struct in_addr* addr)
{
    ...
    if (arp == NULL) {
        error("Failed to open file (/proc/net/arp)");
    }
...
static void send_wol(const char* bcast, unsigned long port, bool ipv6)
{
    ...
    if (ipv6) {
        ...
        if (inet_pton(AF_INET6, bcast, &(addr6.sin6_addr)) != 1) {
        ...
        if (inet_pton(AF_INET, bcast, &(addr.sin_addr)) != 1) {
